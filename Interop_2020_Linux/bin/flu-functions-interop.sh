...
# Function that unzips, sends files to another folder, and archives the zip folder, preserving it
function unzipFolder {
	
	# Usage: unzipFolder fileArray
...
# Function that can be called to check for the uploaded ODLCs (must be run after logging in)
function checkODLCs {

	# Usage: checkODLCs
	
	# Sends a HTTP GET request to the ./api/odlcs address and outputs to a json file
	curl -b $user/bin/auth.txt -H 'Content-Type: application/json' -o $user/bin/objects-total.json http://$localip:$port/api/odlcs

}

# Function that is used to upload data to the Interop Server
function uploadData {

	# Usage: uploadData fileExtension fileArray
	...
	then
		...
		curl -L -b $user/bin/auth.txt -H 'Content-Type: application/json' -d "@$fileName.json" --output $user/bin/object-temp.json http://$localip:$port/api/odlcs
	...
	then
		...
		curl -L -b $user/bin/auth.txt -H 'Content-Type: image/jpeg' --data-binary "@$fileName.jpeg" http://$localip:$port/api/odlcs/$id/image
	...
	then	
		...
		cd $user/bin/plane-data/extracted-data
		curl -L -b $user/bin/auth.txt -H 'Content-Type: image/png' --data-binary "@$fileName.png" http://$localip:$port/api/odlcs/$id/image
	else
...
# Function to get the parameters from the login json file (required for login)
function getParams {
...
# Function to login to the Interop Server and retrieve mission details
